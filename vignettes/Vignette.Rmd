...
The default parameter arguments for the built-in model are:
...
* $\mu N$ represents the number of births into the population per unit time (we assume everyone is born suceptible)
...
Note the `round(..., 3)` function rounds the parameter value to 3 decimal points. This improves readability for the shiny app slider scale.
...
The SIRS model is similar to the SIR model in that individuals become immune to the disease once they recover. However, instead of remainining immune for life (i.e. staying in the $R$ class), they can instead lose this immunity (at rate $\delta$) and re-enter the susceptible class. The equations are given by
